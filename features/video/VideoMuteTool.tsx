...
import { LuVolumeX, LuDownload, LuRefreshCw } from "react-icons/lu";
...
export default function VideoMuteTool() {
  ...
  const mute = async () => {
    ...
    try {
      ...
      setStatus("error");
      setError(
        e instanceof Error ? `Mute failed: ${e.message}` : "Mute failed."
      );
    }
  };
      ...
      {!file ? (
        ...
        >
          ...
          <div className="dropzone-inner">
            <LuVolumeX className="dropzone-icon" />
        ...
        <>
          ...
          {status === "done" && (
            <div className="card" style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <LuVolumeX style={{ flexShrink: 0, color: "var(--muted-foreground)" }} />
              <span className="text-sm text-muted-foreground">
                Ready — click <strong>Download muted video</strong> above to save.
              </span>
            </div>
          )}
        </>
